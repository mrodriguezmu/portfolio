...
// A continuación vamos a generar la "exposición" de los bancos en 2009-2 y 2009-3. La definición de "exposición" es distinta a la utilizada en el dofile "análisis descriptivo credit debt substitution". En este caso, la exposición está dada por cuánto representan los creditos del Top 25% de firmas que más desembolsos de crédito externo tuvo en 2009-3 sobre el total de cartera comercial del banco.

// primero haremos una base que contenga la cartera total de cada banco en 2009-2 con las firmas Top 25% de 2009-3
...
// vamos a marcar el top 25% de las firmas que sacaron cred externo en 2009-3

preserve

	use "$input\top_firmas_cred_externo.dta", clear

	keep if Fecha_tri == yq(2009, 3)

	keep identif top_externo

	rename top_externo firmas_2009_3
...
// conservamos solo las observaciones del 2009-2 (periodo anterior al evento)

keep if Fecha_tri == yq(2009, 2)
...
save "$input\cartera_de_bancos_con_top_firmas_externo_2009_2.dta", replace // base de datos que contiene la cartera total de cada banco con las firmas top 25%. Esta cartera es solo para el trimestre 2009-2 (trimestre anterior al evento)

// ahora veamos la cartera total de cada banco en 2009-2
...
save "$input\cartera_bancos_2009_2_y_2009_3.dta", replace // base de datos que contiene la cartera comercial total de los bancos en 2009-2 y 2009-3

// mergeamos con la base que contiene la cartera solo con el top 25% 

merge 1:1 codentid using "$input\cartera_de_bancos_con_top_firmas_externo_2009_2.dta"
...
gen exposicion_resc = exposicion / .4964583
...
save "$input\exposicion_y_colores_bancos_2009_3.dta", replace // base de datos que contiene las exposiciones y la paleta de colores para cada banco en 2009-2 y 2009-3


**# 2) GRÁFICAS BANCOS DOMÉSTICOS 2009-3

**# 2.1) PARA EL TOP 25%

// vamos a generar una base que contenga los nuevos créditos que cada banco le otorgó en 2009-2 y 2009-3 al Top 25% de firmas que más desembolsos de crédito externo tomó en 2009-3
...
keep if Fecha_tri == yq(2009, 2) | Fecha_tri == yq(2009, 3)

// mergeamos con la base que tiene los nits del Top 25% de 2009-3

merge m:1 identif using "$input\top_firmas_cred_externo_2009_3.dta"
...
merge 1:1 codentid Fecha_tri using "$input\cartera_bancos_2009_2_y_2009_3.dta"

drop _merge

// guardemos esta base 

save "$input\base_graficas_bancos_expo_2009_2_2009_3.dta", replace // base de datos que contiene los nuevos creditos otorgados por cada banco al Top 25% y la cartera comercial total de los bancos en 2009-2 y 2009-3.
...
// nos quedamos solo con los trimestres 2009-2 y 2009-3
...
merge 1:1 codentid Fecha_tri using "$input\base_graficas_bancos_expo_2009_2_2009_3.dta"
...
merge m:1 codentid using "$input\exposicion_y_colores_bancos_2009_3.dta"
...
// nos quedamos solamente con el trimestre 2009-3
...
graph bar lag_prop_top25_USD prop_top25_USD if exposicion_USD != ., over(corto, sort(exposicion) label(angle(45) labsize(vsmall))) bar(1, color("blue")) bar(2, color("blue")) ytitle("Porcentaje") graphregion(color(white)) plotregion(color(white) style(none)) note("Se ordenan los bancos según su nivel de exposición; de menos expuestos (izquierda) a más" "expuestos (derecha)." "Los bancos más expuestos son aquellos para los cuales la cartera comercial con firmas que" "hacen parte del Top 25% que más crédito externo sacó en 2009-3 representó un mayor por" "centaje de su cartera comercial total." "Para cada banco, la barra de la izquierda corresponde al trimestre 2009-2, mientras que la de" "la derecha corresponde a 2009-3." "Los bancos omitidos no cuentan con cartera comercial en dólares.") legend(off) ylabel(, angle(0))

graph export "$output\nuevo credito dolares como porcentaje de cartera bancos 2009.png", replace
...
graph export "$output\nuevo credito pesos como porcentaje de cartera bancos 2009.png", replace

**# 2.1.4) DESEMBOLSOS CRED EXTERNO TOP 25% COMO PORCENTAJE DE CARTERA TOTAL
...
graph bar lag_prop_exter prop_exter, over(corto, sort(exposicion) label(angle(45) labsize(vsmall))) bar(1, color("blue")) bar(2, color("blue")) ytitle("Porcentaje") graphregion(color(white)) plotregion(color(white) style(none)) note("Se ordenan los bancos según su nivel de exposición; de menos expuestos (izquierda) a más" "expuestos (derecha)." "Los bancos más expuestos son aquellos para los cuales la cartera comercial con firmas que" "hacen parte del Top 25% que más crédito externo sacó en 2009-3 representó un mayor por" "centaje de su cartera comercial total." "Para cada banco, la barra de la izquierda corresponde al trimestre 2009-2, mientras que la de" "la derecha corresponde a 2009-3.") legend(off) ylabel(, angle(0))

graph export "$output\desembolsos como porcentaje de cartera bancos 2009.png", replace


**# 2.2) PARA LAS DEMÁS FIRMAS

// vamos a generar una base que contenga los nuevos créditos que cada banco le otorgó en 2009-2 y 2009-3 a las firmas distintas al Top 25% de firmas que más desembolsos de crédito externo tomó en 2009-3
...
graph export "$output\nuevo credito como porcentaje de cartera bancos otras firmas 2009.png", replace
...
graph export "$output\nuevo credito dolares como porcentaje de cartera bancos otras firmas 2009.png", replace
...
// Por último, generaremos una grafica que muestre el total de desembolsos de crédito externo y nuevos créditos doméstico de todas las firmas que tomaron desembolsos de crédito externo en 2009-3. El periodo de tiempo para esta gráfica será de 2007-1 a 2010-1
...
// conservamos solamente los periodos que nos interesan que son de 2007-1 a 2010-1

keep if Fecha_tri >= yq(2007,1) & Fecha_tri <= yq(2010,1) 
...
merge m:1 identif using "$input\nits_cred_externo_2009_3.dta" // mergeamos con la base que contiene los nits de todas las firmas con desembolsos en 2009-3
