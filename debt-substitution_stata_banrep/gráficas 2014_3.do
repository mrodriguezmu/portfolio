...
// A continuación vamos a generar la "exposición" de los bancos en 2014-2 y 2014-3. La definición de "exposición" es distinta a la utilizada en el dofile "análisis descriptivo credit debt substitution". En este caso, la exposición está dada por cuánto representan los creditos del Top 25% de firmas que más desembolsos de crédito externo tuvo en 2014-3 sobre el total de cartera comercial del banco.

// primero haremos una base que contenga la cartera total de cada banco en 2014-2 con las firmas Top 25% de 2014-3
...
save "$input\cartera_de_bancos_con_top_firmas_externo.dta", replace // base de datos que contiene la cartera total de cada banco con las firmas top 25%. Esta cartera es solo para el trimestre 2014-2 (trimestre anterior al evento)

// ahora veamos la cartera total de cada banco en 2014-2
...
save "$input\cartera_bancos_2014_2_y_2014_3.dta", replace // base de datos que contiene la cartera comercial total de los bancos en 2014-2 y 2014-3

// mergeamos con la base que contiene la cartera solo con el top 25% 

merge 1:1 codentid using "$input\cartera_de_bancos_con_top_firmas_externo.dta"
...
gen exposicion_resc = exposicion / .1213465
...
save "$input\exposicion_y_colores_bancos.dta", replace // base de datos que contiene las exposiciones y la paleta de colores para cada banco en 2014-2 y 2014-3


**# 2) GRÁFICAS BANCOS DOMÉSTICOS 2014-3

**# 2.1) PARA EL TOP 25%

// vamos a generar una base que contenga los nuevos créditos que cada banco le otorgó en 2014-2 y 2014-3 al Top 25% de firmas que más desembolsos de crédito externo tomó en 2014-3
...
// mergeamos con la base que tiene los nits del Top 25% de 2014-3

merge m:1 identif using "$input\top_firmas_cred_externo_2014_3.dta"
...
merge 1:1 codentid Fecha_tri using "$input\cartera_bancos_2014_2_y_2014_3.dta"

drop _merge

// guardemos esta base 

save "$input\base_graficas_bancos_expo_2014_2_2014_3.dta", replace // base de datos que contiene los nuevos creditos otorgados por cada banco al Top 25% y la cartera comercial total de los bancos en 2014-2 y 2014-3.
...
// nos quedamos solo con los trimestres 2014-2 y 2014-3
...
merge 1:1 codentid Fecha_tri using "$input\base_graficas_bancos_expo_2014_2_2014_3.dta"

drop _merge

replace desem_exter = 0 if desem_exter == .

// eliminamos al banco 51, ya que no aparece en 2014-3
...
merge m:1 codentid using "$input\exposicion_y_colores_bancos.dta"
...
// nos quedamos solamente con el trimestre 2014-3
...
graph bar lag_prop_top25_USD prop_top25_USD if exposicion_USD != ., over(corto, sort(exposicion) label(angle(45) labsize(vsmall))) bar(1, color("blue")) bar(2, color("blue")) ytitle("Porcentaje") graphregion(color(white)) plotregion(color(white) style(none)) note("Se ordenan los bancos según su nivel de exposición; de menos expuestos (izquierda) a más" "expuestos (derecha)." "Los bancos más expuestos son aquellos para los cuales la cartera comercial con firmas que" "hacen parte del Top 25% que más crédito externo sacó en 2014-3 representó un mayor por" "centaje de su cartera comercial total." "Para cada banco, la barra de la izquierda corresponde al trimestre 2014-2, mientras que la de" "la derecha corresponde a 2014-3." "Los bancos omitidos no cuentan con cartera comercial en dólares.") legend(off) ylabel(, angle(0))

graph export "$output\nuevo credito dolares como porcentaje de cartera bancos.png", replace
...
graph export "$output\nuevo credito pesos como porcentaje de cartera bancos.png", replace
...
graph export "$output\desembolsos como porcentaje de cartera bancos.png", replace

// sin outliers (bancos para los cuales el porcentaje es mayor a 100%)
...
graph bar lag_prop_exter prop_exter, over(corto, sort(exposicion) label(angle(45) labsize(vsmall))) bar(1, color("blue")) bar(2, color("blue")) ytitle("Porcentaje") graphregion(color(white)) plotregion(color(white) style(none)) note("Se ordenan los bancos según su nivel de exposición; de menos expuestos (izquierda) a más" "expuestos (derecha)." "Los bancos más expuestos son aquellos para los cuales la cartera comercial con firmas que" "hacen parte del Top 25% que más crédito externo sacó en 2014-3 representó un mayor por" "centaje de su cartera comercial total." "Para cada banco, la barra de la izquierda corresponde al trimestre 2014-2, mientras que la de" "la derecha corresponde a 2014-3." "Se omiten los bancos para los cuales el porcentaje es mayor a 100%.") legend(off) ylabel(, angle(0))

graph export "$output\desembolsos como porcentaje de cartera bancos sin outliers.png", replace


**# 2.2) PARA LAS DEMÁS FIRMAS

// vamos a generar una base que contenga los nuevos créditos que cada banco le otorgó en 2014-2 y 2014-3 a las firmas distintas al Top 25% de firmas que más desembolsos de crédito externo tomó en 2014-3
...
graph export "$output\nuevo credito como porcentaje de cartera bancos otras firmas.png", replace
...
graph export "$output\nuevo credito dolares como porcentaje de cartera bancos otras firmas.png", replace
...
// Por último, generaremos una grafica que muestre el total de desembolsos de crédito externo y nuevos créditos doméstico de todas las firmas que tomaron desembolsos de crédito externo en 2014-3. El periodo de tiempo para esta gráfica será de 2013-4 a 2016-4
...
merge m:1 identif using "$input\nits_cred_externo_2014_3.dta" // mergeamos con la base que contiene los nits de todas las firmas con desembolsos en 2014-3
